...
export default function BoardClient({ sessionId }: BoardClientProps) {
  ...
  return <BoardInner sessionId={sessionId} />;
}

function BoardInner({ sessionId }: BoardClientProps) {
  ...
  const [userId] = useState<string>(keycloak.tokenParsed?.sub ?? "user-001");
...
//   if (loading) return null; // ADD THIS — blocks render until auth confirmed
